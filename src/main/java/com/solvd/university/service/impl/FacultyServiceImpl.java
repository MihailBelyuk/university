...
import com.solvd.university.dao.impl.FacultyDaoImpl;
...
public class FacultyServiceImpl implements IFacultyService {
    ...
    @Override
    public void addFacultyRecord(Faculty faculty, Long deansId, Long universitiesId) {
    ...
    public Faculty findFacultyById(Long id) {
        return facultyDao.findById(id).orElseThrow(() -> new ResourceNotFoundException("Failed to find faculty with id " + id));
    }

    @Override
    public void updateFacultyRecord(Faculty faculty, Long deansId, Long universitiesId) {
        ...
        showAllFaculties().stream()
                .filter(faculty1 -> faculty1.getId().equals(faculty.getId()))
                .findFirst()
                .ifPresentOrElse(dean1 -> facultyDao.update(faculty, deansId, universitiesId), () -> addFacultyRecord(faculty, deansId, universitiesId));
    }

    @Override
    public void deleteFaculty(Faculty faculty) {
        facultyDao.delete(showAllFaculties().stream()
                .filter(faculty1 -> faculty1.getId().equals(faculty.getId()))
                .findAny()
